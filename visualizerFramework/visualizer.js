...
```
## toggleEscapeMenu()
On a phone the ƒ⁻¹ control expands the otherwise-hidden app-level destinations.  It is
an escape relation rather than a Back/Undo command: one visual state can expose several
possible destinations.
```javascript
*/
export function toggleEscapeMenu () {
   const $menu = $('#top-right-menu');
   const open = !$menu.hasClass('escape-open');
   $menu.toggleClass('escape-open', open);
   $('#escape-hatch').attr('aria-expanded', open ? 'true' : 'false');
}

export function closeEscapeMenu () {
   $('#top-right-menu').removeClass('escape-open');
   $('#escape-hatch').attr('aria-expanded', 'false');
}
...
```
## help()
Link to the visualizer-specific help page
```javascript
*/
